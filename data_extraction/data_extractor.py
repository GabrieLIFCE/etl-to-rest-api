...
from data_extraction.extractor_utils import read_json_file
from datetime import datetime

database_name = 'database.sqlite'
basedir = os.path.abspath(os.path.dirname(__file__)).replace('data_extraction', '')
base = declarative_base()
engine = create_engine('sqlite:///'+os.path.join(basedir, database_name), echo=False)
...
class DataCreating():    
    ...
    class CVSSV3(base):
        ...
        cvssv3_score =  Column('cvssv3_score', Numeric, unique=False)

        def __init__(self, id_cve, version, vector_string, attack_vector, cvssv3_score):
            self.id_cve = id_cve
    ...
    def remove_database(cls):
        _path = os.path.join(basedir, database_name)
    ...
    def cvssv3_parser(cls, id_cve:str, cve_object:dict): 
        ...
        try: cvssv3_score = cve_object['impact']['baseMetricV3']['cvssV3']['baseScore']
        except: cvssv3_score = None

        cvssv3_model = DataCreating.CVSSV3(id_cve, version, vector_string, attack_vector, cvssv3_score)
        return cvssv3_model
...
def populate_database():
    if os.path.exists(os.path.join(basedir, database_name)):return
